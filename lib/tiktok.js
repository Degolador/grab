...
/**
 * Resolve e expande links curtos do TikTok (vt.tiktok.com, vm.tiktok.com, /t/, etc)
 */
async function resolveTikTokUrl(inputUrl, depth = 0) {
    if (!inputUrl) return inputUrl;
    let url = inputUrl.trim();

    // Se já tiver o ID direto de vídeo ou foto, limpa parâmetros de rastreamento da URL
    if (/\/(video|photo)\/\d+/i.test(url)) {
        return url.split("?")[0];
    }

    const isShort = /(vt\.tiktok\.com|vm\.tiktok\.com|\/t\/|vxtiktok\.com|tnktok\.com)/i.test(url);
    if (!isShort || depth > 5) return url;

    // 1. Tenta descurtar lendo os cabeçalhos de redirecionamento HTTP 301/302 (manual)
    try {
        ...
        if (location) {
            if (/\/(video|photo)\/\d+/i.test(location)) {
                return location.split("?")[0];
            }
            if (/(vt\.tiktok\.com|vm\.tiktok\.com|\/t\/)/i.test(location)) {
                return await resolveTikTokUrl(location, depth + 1);
            }
            if (location !== "https://www.tiktok.com/" && location !== "https://www.tiktok.com/?_r=1") {
                return location;
            }
        }
    } catch {}

    // 2. Fallback: Segue os redirecionamentos até o destino final
    try {
        ...
        if (resFollow.url && /\/(video|photo)\/\d+/i.test(resFollow.url)) {
            return resFollow.url.split("?")[0];
        }
    } catch {}
...
async function fetchTikTokMetadata(targetUrl) {
    ...
    const urlsToTry = Array.from(new Set([cleanUrl, targetUrl]));
    ...
    for (const u of urlsToTry) {
        ...
        try {
            const res = await fetch("https://www.tikwm.com/api/", {
                ...
                body: new URLSearchParams({ url: u, count: 12, cursor: 0, web: 1, hd: 1 })
            });
        ...
        try {
            const res = await fetch(`https://api.tiklydown.eu.org/api/download?url=${encodeURIComponent(u)}`);
            const json = await res.json();
            if (json && json.video) {
                return {
...
async function downloadTikTok(options) {
    ...
    if (onLog) onLog("Expandindo e descurtando link do TikTok...");
    const resolvedUrl = await resolveTikTokUrl(url);

    if (onLog) onLog("Obtendo informações da mídia (sem marca d'água)...");
    ...
    if (format === "audio") {
        ...
        if (onLog) onLog(`Baixando áudio MP3...`);
        try {
