...
const { downloadTikTok, resolveTikTokUrl } = require("./tiktok");
...
function downloadMedia(options) {
    ...
    return new Promise(async (resolve, reject) => {
        const isTikTok = (platform && platform.id === "tiktok") || /(tiktok\.com|vt\.tiktok\.com|vm\.tiktok\.com|vxtiktok\.com|tnktok\.com)/i.test(url);
        if (isTikTok) {
            try {
                const tiktokResult = await downloadTikTok(options);
                return resolve(tiktokResult);
            } catch (err) {
                if (onLog) onLog(`Nota: Extração direta TikTok via API: ${err.message}. Tentando via yt-dlp...`);
            }
        }

        const outputDir = getOutputDir(format);

        async function executeYtDlp(retryCount = 0, withThumbnail = true) {
            let targetUrl = url;
            if (isTikTok) {
                targetUrl = await resolveTikTokUrl(url);
            }
